...
def gen_balmer_lines():
    ...
    for lvl in range(3, 15):
        ...
        lam_vac_ang = lam_vac*1e10  # meters to angstroms 
        # since the rydberg const above is in (1/m)

        #print("Transition:", lvl, "--> 2,       wavelength in vacuum [Angstroms]:")
        #print("{:.3f}".format(lam_vac_ang))
...
def add_spec(lam_em, llam_em, lerr, old_llam, old_llamerr, 
    num_points, num_galaxies, lam_grid, lam_step):
    
    for i in range(len(lam_grid)):
        
        # add fluxes
        new_ind = np.where((lam_em >= lam_grid[i] - lam_step/2) & \
                           (lam_em <  lam_grid[i] + lam_step/2))[0]
        ...
        if new_ind.size:
            ...
            # Only count a galaxy in a particular bin if for 
            # that bin at least one point is positive
            ...
            if len(new_ind) > 1:
                for ind in new_ind:
                    ...
                    if np.isfinite(sig):
                        ...
                        old_llamerr[i].append(noise**2)
                        # keep track of how many points were added to each bin in lam_grid
                        num_points[i] += 1
            ...
            #if np.isfinite(sig):
            #    # Because I put in NaN values where 
            #    # there wasn't anything to add while combining PAs
            #
            ...
            #        errmsg = "This error occurs when the code" + \
            #                 "encounters a negative or zero signal." + "\n" + \
            #                 "This error should not have been triggered if" + \
            #                 "you're using PA combined PEARS spectra or " + \
            #                 "the FIGS spectra read in with the get_figs_data(...)" + \
            #                 "function in grid_coadd.py. "
            #                 "These flux points should've been taken out by the" + \
            #                 "code that combines spectra " + \
            #                 "for each galaxy at different PAs and the function" + \
            #                 "that returns FIGS data." + "\n" + \
...
def rescale(id_arr_cell, field_arr_cell, z_arr_cell, dl_tbl):
    ...
    for k in range(len(id_arr_cell)):
        ...
        # FIGS data
        #lam_obs, flam_obs, ferr_obs, return_code = \
...
def create_stacks(cat, urcol, z_low, z_high, z_indices, start):
    ...
    # because the code below expects log(stellar mass)
    stellar_mass = np.log10(cat['zp_ms'][z_indices])

    # Read in dl lookup table
    # Required for deredshifting
    dl_tbl = np.genfromtxt(massive_galaxies_dir + 'cluster_codes/dl_lookup_table.txt', 
    ...
    lam_step = 10  # somewhat arbitrarily chosen # pretty much trial and error
    ...
    # This redshift range was chosen so that the 4000A break would 
    # fall in the observed wavelength range
    ...
    for col in np.arange(col_low, col_high, col_step):
        for ms in np.arange(mstar_low, mstar_high, mstar_step):
            ...
            # This function returns the median of the median values 
            # (in the given band) from all given spectra
            # All spectra to be coadded in a given grid cell need to be divided by this value
            medarr, medval, stdval = \
            rescale(pears_id[indices], pears_field[indices], zp[indices], dl_tbl)
            ...
            for u in range(len(pears_id[indices])):
                ...
                # Multiplying by median value of the flux medians 
                # to get it back to physical units
                ...
                add_spec(pears_lam_em, pears_llam_em, pears_lerr, 
                    pears_old_llam, pears_old_llamerr, 
                    pears_num_points, pears_num_galaxies, lam_grid, lam_step)

                figs_old_llam, figs_old_llamerr, figs_num_points, figs_num_galaxies = \
                add_spec(figs_lam_em, figs_llam_em, figs_lerr, 
                    figs_old_llam, figs_old_llamerr,
            ...
            pears_old_llam, pears_old_llamerr = \
            take_median(pears_old_llam, pears_old_llamerr, lam_grid)
            figs_old_llam, figs_old_llamerr = \
            take_median(figs_old_llam, figs_old_llamerr, lam_grid)
            ...
            for u in range(len(pears_id[indices])):
                ...
                # ----------------------------- Get data ----------------------------- #
                grism_lam_obs, grism_flam_obs, grism_ferr_obs, 
                pa_chosen, netsig_chosen, return_code = \
    ...
    final_fits_filename = 'stacks_' + str(z_low).replace('.','p') + '_' + \
                          str(z_high).replace('.','p') + '.fits'
...
def get_avg_col_mass_arrays(ur_color, stellar_mass, stack_hdu):
    ...
    # While these are useful numbers to have, 
    # they are currently only used in the plotting routine.
    cellcount = 0
...
def plot_stacks(cat, urcol, z_low, z_high, z_indices, start):
    ...
    for i in np.arange(col_low, col_high, col_step):
        for j in np.arange(mstar_low, mstar_high, mstar_step):
            ...
            if indices.size:
                ...
                #medarr, medval, stdval = rescale(figs_id[indices], 
                #                         figs_field[indices], zp[indices], dl_tbl)
                medarr, medval, stdval = rescale(pears_id[indices], 
                    pears_field[indices], zp[indices], dl_tbl)
            else:
            ...
            if (len(indices) < 5):
                ...
                print("Too few spectra in stack (i.e., less than 5).") 
                print("Continuing to the next grid cell...")
                cellcount += 1
            ...
            for u in range(len(pears_id[indices])):
                ...
                # Plotting
                ax.plot(pears_lam_em, pears_llam_em, ls='-', 
                    color='paleturquoise', linewidth=0.5)
            ...
            ax.errorbar(lam, pears_llam, yerr=pears_llam_err, fmt='.-', 
                        color='mediumblue', linewidth=0.5,\
                        elinewidth=0.2, ecolor='r', markeredgecolor='mediumblue', 
                        capsize=0, markersize=0.5, zorder=5)

            #figs_llam_zero_idx = np.where(figs_llam == 0.0)[0]
            #figs_llam[figs_llam_zero_idx] = np.nan
            #ax.errorbar(lam, figs_llam, yerr=figs_llam_err, fmt='.-', 
            #            color='darkorange', linewidth=0.5,\
            #            elinewidth=0.2, ecolor='r', markeredgecolor='darkorange', 
            #            capsize=0, markersize=0.5, zorder=5)
    ...
    fig.savefig(stacking_figures_dir + final_fits_filename.replace('.fits','.pdf'), 
...
def stack_plot_massive(cat, urcol, z_low, z_high, z_indices, start):
    ...
    indices = np.where((ur_color >= col_lim_low) & (ur_color < col_lim_high) &\
    ...
    #to_check = [(109151, 'GOODS-S'), (56663, 'GOODS-N'), 
    #(21592, 'GOODS-S'), (109794, 'GOODS-S'), (47814, 'GOODS-N')]
    ...
    for u in range(len(pears_id[indices])):
        ...
        print("Spec-z, photo-z, and redshift error for this galaxy:")
        print(current_spec_redshift, current_redshift, zerr)
        ...
        pears_llam_em_rebinned = interpolate.griddata(points=pears_lam_em, 
            values=pears_llam_em, 
        ...
        g102_flam_norm = g102_flam_obs / np.mean(g102_flam_obs)
        g102_ferr_norm = g102_ferr_obs / np.mean(g102_flam_obs)

        ## Mask lines
        """
        #mask_indices_grism = get_mask_indices(grism_lam_obs, current_redshift)
        #mask_indices_g102  = get_mask_indices(g102_lam_obs, current_redshift)
        ...
        #mask_indices_grism = np.delete(arr=mask_indices_grism, obj=remove_mask_idx)

        #remove_mask_idx = np.where(mask_indices >= len(g102_lam_obs))[0]
        #mask_indices_g102 = np.delete(arr=mask_indices_g102, obj=remove_mask_idx)
        ...
        cont_div_flux_g800l = grism_flam_norm / splev(grism_lam_obs, spl)
        cont_div_err_g800l  = grism_ferr_norm / splev(grism_lam_obs, spl)

        # ------- FIGS
        # SciPy smoothing spline fit
        spl = splrep(x=g102_lam_obs, y=g102_flam_norm, k=3, s=0.2)
        wav_plt = np.arange(g102_lam_obs[0], g102_lam_obs[-1], 1.0)
        spl_eval = splev(wav_plt, spl)

        # Divide the given flux by the smooth spline fit
        cont_div_flux_g102 = g102_flam_norm / splev(g102_lam_obs, spl)
        cont_div_err_g102  = g102_ferr_norm / splev(g102_lam_obs, spl)
        ...
        ax1.plot(grism_lam_obs, grism_flam_norm, 'o-', 
                 markersize=3.0, color='k', linewidth=1.5, label='PEARS obs data')
        ax1.fill_between(grism_lam_obs, grism_flam_norm - grism_ferr_norm, 
                         grism_flam_norm + grism_ferr_norm, \
        ...
        #ax1.plot(pears_lam_em, p_pears(pears_lam_em), 
        #         color='teal', zorder=1.0, label='AstroPy polynomial fit')
        #ax1.plot(figs_lam_em,  p_figs(figs_lam_em), 
        #         color='brown', label='FIGS obs data')
        #ax1.plot(pears_lam_em, np_polynomial(pears_lam_em), 
        #         color='crimson', zorder=1.0, label='NumPy polynomial fit')
        #ax1.plot(pears_lam_em, rebin_polynomial(pears_lam_em), 
        #         lw=3.0, zorder=2.0, color='tab:brown', label='NumPy rebin polynomial fit')
        """
        ...
        for l in range(len(all_lines)):
            ...
            #xlab_line = (lam_em_formaskplot[current_line_idx] - lam_grid_low) / \
            (lam_grid_high - lam_grid_low)
        ...
        ax2.plot(grism_lam_obs, cont_div_flux, 
            color='teal', lw=2.0, label='Continuum divided flux')
        ...
        scaling_fac = np.mean(cont_div_flux_g800l[shift_idx])
        cont_div_flux_g800l /= scaling_fac

        shift_idx = np.where((figs_lam_em >= 4600) & (figs_lam_em <= 4700))[0]
        scaling_fac = np.mean(cont_div_flux_g102[shift_idx])
        cont_div_flux_g102 /= scaling_fac

        # add the continuum subtracted spectrum
        pears_old_llam, pears_old_llamerr, pears_num_points, pears_num_galaxies = \
        add_spec(pears_lam_em, cont_div_flux_g800l, cont_div_err_g800l, 
            pears_old_llam, pears_old_llamerr, 
            pears_num_points, pears_num_galaxies, lam_grid, lam_step)

        figs_old_llam, figs_old_llamerr, figs_num_points, figs_num_galaxies = \
        add_spec(figs_lam_em, cont_div_flux_g102, cont_div_err_g102, 
            figs_old_llam, figs_old_llamerr, 
            figs_num_points, figs_num_galaxies, lam_grid, lam_step)

        ax.plot(pears_lam_em, cont_div_flux_g800l, ls='-', 
            color='turquoise', linewidth=0.5, alpha=0.4)
        ax.plot(figs_lam_em, cont_div_flux_g102, ls='-', 
    ...
    ax.fill_between(lam_grid, pears_old_llam - pears_old_llamerr, 
                    pears_old_llam + pears_old_llamerr, \
                    color='gray', alpha=0.5, zorder=5)

    ax.plot(lam_grid, figs_old_llam, '.-', color='darkorange', linewidth=1.5, \
        markeredgecolor='darkorange', markersize=1.0, zorder=5)
    ax.fill_between(lam_grid, figs_old_llam - figs_old_llamerr, 
                    figs_old_llam + figs_old_llamerr, \
                    color='gray', alpha=0.5, zorder=5)
    ...
    for q in range(len(lam_grid)):
        ...
        fh.write("{:.2f}".format(lam_grid[q]) + " " + 
                 "{:.6f}".format(pears_old_llam[q]) + " " + 
                 "{:.6f}".format(pears_old_llamerr[q]))
        fh.write("\n")
    fh.close()
    ...
    #ax.errorbar(lam_grid, figs_old_llam, yerr=figs_old_llamerr, fmt='.-', 
    #            color='darkorange', linewidth=2.5,
    #            elinewidth=1.0, ecolor='r', markeredgecolor='darkorange', 
    #            capsize=0, markersize=4.0, zorder=5)
    ...
    #ax.text(0.67, 0.26, 'PEARS ACS/G800L', 
    #        verticalalignment='top', horizontalalignment='left', \
    #        transform=ax.transAxes, color='royalblue', size=20)
    #ax.text(0.67, 0.195, 'FIGS WFC3/G102', 
    #        verticalalignment='top', horizontalalignment='left', \
    #        transform=ax.transAxes, color='darkorange', size=20)

    # Measure Mg/Fe
    #mg2fe = fit_gauss_mgfe_astropy(lam_grid, pears_old_llam, 
    # num_massive, z_low, z_high, ms_lim_low, ms_lim_high)
...
def get_all_line_wav():
    ...
    all_lines = [oii3727, hbeta, oiii4959, oiii5007, halpha, 
                gband, mg2_mgb, fe5270, fe5335]#, fe5406, nad]
    all_line_labels = [r'$[OII]3727$', r'H$\beta$', r'$[OIII]4959$', r'$[OIII]5007$',
                       r'H$\alpha + [NII]$', 'G-band', r'$\mathrm{Mg_2 + Mgb}$', 
                       'Fe5270', 'Fe5335']#, 'Fe5406', 'NaD+TiO']
...
def fit_gauss_mgfe_astropy(stack_lam, stack_llam, num_massive, 
    z_low, z_high, ms_lim_low, ms_lim_high):
    ...
    # Fitting
    # gauss_init = (const_init - hb_abs_init) + (const_init - mg_abs_init) + \
    #              (const_init - fe_abs_init)
    ...
    ax.plot(stack_lam, g[0](stack_lam) - g[1](stack_lam), ls='--', 
            color='dodgerblue', lw=2, zorder=4)
    ax.plot(stack_lam, g[0](stack_lam) - g[2](stack_lam), ls='--', 
            color='springgreen', lw=2, zorder=4)
    ax.plot(stack_lam, g[0](stack_lam) - g[3](stack_lam), ls='--', 
    ...
    # in angstrom; assumes a mean 3% error on redshifts
    width_z_err = 0.029 * 5175
    # in angstroms; number of ACS pixels of a typical galaxy times spectral res at mgb
    width_lsf_err = 3 * 50
...
def main():
    ...
    #cat = np.genfromtxt(stacking_analysis_dir + 'full_pears_results_chabrier.txt', 
    ...
    # The combined grism+photometry fits, redshifts, and 
    derived parameters are given in v4.1.5 (Momcheva et al. 2016)
    photometry_names = ['id', 'ra', 'dec', 'f_F160W', 'e_F160W', 'f_F435W', 'e_F435W', 
    'f_F606W', 'e_F606W', 'f_F775W', 'e_F775W', 'f_F850LP', 'e_F850LP', 
    'f_F125W', 'e_F125W', 'f_F140W', 'e_F140W', 
    'f_U', 'e_U', 'f_IRAC1', 'e_IRAC1', 'f_IRAC2', 'e_IRAC2', 
    'f_IRAC3', 'e_IRAC3', 'f_IRAC4', 'e_IRAC4', 
    'IRAC1_contam', 'IRAC2_contam', 'IRAC3_contam', 'IRAC4_contam']
    goodsn_phot_cat_3dhst = np.genfromtxt(threedhst_datadir + 
        'goodsn_3dhst.v4.1.cats/Catalog/goodsn_3dhst.v4.1.cat', 
        dtype=None, names=photometry_names,
        usecols=(0,3,4, 9,10, 15,16, 27,28, 39,40, 45,46, 48,49, \
        54,55, 12,13, 63,64, 66,67, 69,70, 72,73, 90,91,92,93), \
        skip_header=3)
    goodss_phot_cat_3dhst = np.genfromtxt(threedhst_datadir + 
        'goodss_3dhst.v4.1.cats/Catalog/goodss_3dhst.v4.1.cat', 
        dtype=None, names=photometry_names, 
        usecols=(0,3,4, 9,10, 18,19, 30,31, 39,40, 48,49, \
        54,55, 63,64, 15,16, 75,76, 78,79, 81,82, 84,85, 130,131,132,133), \
        skip_header=3)
    """
    ...
    # Total time taken
    print("Total time taken for all stacks --")
    print("{:.2f}".format((time.time() - start)/60.0), "minutes.")
