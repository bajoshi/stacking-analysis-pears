...
def get_colors(matchedfile, threedn, threeds):
    ...
    for i in range(len(cat)):
        ...
        threedra.append(cat['threedra'][i])
        threeddec.append(cat['threeddec'][i])
    ...
    return pears_id, fieldname, ur_color, stellarmass, threed_id, threed_z

def concat(pears_id, fieldname, ur_color, stellarmass, threed_id, threed_z, pid, fname, ur, ms, tid, tz):
...
if __name__ == '__main__':
    ...
    pears_id, fieldname, ur_color, stellarmass, threed_id, threed_z = get_colors(cdfn1, threedn, threeds)
    # cdfn1 has to be done separately to get the concatenation to work

    for field in all_fields[1:]:
        pid, fname, ur, ms, tid, tz = get_colors(field, threedn, threeds)
        pears_id, fieldname, ur_color, stellarmass, threed_id, threed_z = concat(pears_id, fieldname, ur_color, stellarmass, threed_id, threed_z, pid, fname, ur, ms, tid, tz)

    outdir = home + '/Desktop/FIGS/stacking-analysis-pears/'
    data = np.array(zip(pears_id, fieldname, ur_color, stellarmass, threed_id, threed_z),\
                    dtype=[('pears_id', int), ('fieldname', '|S7'), ('ur_color', float), ('stellarmass', float), ('threed_id', int), ('threed_z', float)])

    np.savetxt(outdir + 'color_stellarmass.txt', data, fmt=['%d', '%s', '%.2f', '%.2f', '%d', '%.2f'], delimiter=' ',\
               ...
               'pearsid field urcol mstar threedid threedzphot')