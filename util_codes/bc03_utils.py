...
def read_current_filepos(filehandle, dt='i', number=1):
    """
    This function reads the given number of binary
    data which is of the supplied type from the specified
    filehandle. The default number of elements to extract is one
...
def get_stuff_from_ised(modelfile):
    ...
    ignore = read_current_filepos(fh)
    # "ignore" is the number 1208 for SSPs and depends on Tcut for CSPs
    totalages = read_current_filepos(fh)[0]
    # totalages should be 221 for the SSP models

    """
    FOR CSPs ----
    The number of ages you get in the previous line depends on the
    CUT OFF TIME set in the csp run.
    The max age of any model that is given by BC03 is 20 Gyr.
    I found by trial and error that if the
    ---> Tcut is set to above 20 Gyr then it gives 221 ages
         which is the same as that given by a SSP.
    ---> Tcut is set to undet 20 Gyr but more than 13.8 Gyr
         then it gives a number (for totalages) that is between 221 and 245.
    ---> Tcut is set to 13.8 Gyr then it gives 245 ages.
    ---> Tcut is set to 5 Gyr then it gives 261 ages.
    Keep in mind however that the total age range still
    remains between 0 to 20 Gyr.
    Also I'm not sure if having a lower/higher number of totalages
    (than 245) will have any effect on my analysis.
    ...
    if 'salp' in modelfile:
        fh.seek(328, 1)
        """
        Done by trial and error and looking at the ezgal code
        but mostly the ezgal code helped.
        This goes 328 bytes forward from the current position.
        If the second keyword is not provided then it assumes
        absolute positioning.
        I think (although I couldn't see all the characters
        properly) that these 328 bytes simply contain their
        copyright string (or something like that).
        Couldn't quite tell properly because they won't
        show up properly (some unicode issue).
        """
        ...
        if iseg > 0:
           junk = read_current_filepos(fh, dt='f', number=6*iseg)  # noqa
        ...
        junk = read_current_filepos(fh)
        junk = read_current_filepos(fh, number=3)  # noqa

    # t2=time.time()
    # print("Finished prep for looping over seds.
    #       Time taken:", "{:.3f}".format(t2-t1), "seconds.")
    ...
    # t3 = time.time()
    # print("Starting to loop over seds.
    #       Time taken:", "{:.3f}".format(t3-t2), "seconds.")

    for i in range(totalages):
        ignore = read_current_filepos(fh, number=2)
        nlam = read_current_filepos(fh)  # noqa

        # temp = read_current_filepos(fh, dt='f', number=totalwavelengths)
        # if not temp.size:
        #     print("Working with isedfile:", modelfile)
        #     print("Iteration:", i, "    nlam:", nlam,
        #           "    age:", allages[i],
        #           "    SED array shape and size from fromfile:",
        ...
        ignore = read_current_filepos(fh, dt='f', number=num)  # noqa
...
def call_cspgalaxev(isedfile, tau, output, dust='N',
                    z='0', sfh='1', recycle='N', tcut='20.0', verbose=False):
    """
    ...
    You need this, i.e., Popen, instead of
    ...
    This is different from passing arguments to the process on
    the command line.
    ...
    It must be given the args interactively after the
    csp_galaxev program is called.
    ...
    An optional stdout argument may also be given. This can be a
    logfile if you want.
    ...
    start = subprocess.Popen([home +
                              '/Documents/GALAXEV2016/bc03/src/csp_galaxev'],
    ...
    communicate_params = os.linesep.join([isedfile, dust, z, sfh, str(tau),
                                          recycle, tcut, output])
    ...
    # Call the process
    params = start.communicate(input=communicate_params)  # noqa
...
def gen_bc03_spectrum(tau, metals, outdir_ised):
    ...
    if metals == 0.0001:
        metallicity = 'm22'
        isedfile = (home + "/Documents/GALAXEV2016/bc03/src/"
    ...
    output = (outdir_ised + "bc2003_hr_" + metallicity
              + "_csp_tau" + tau_str + "_chab")
    ...
    if not os.path.isfile(output + '.ised'):
        ...
        t2 = time.time()
        print("Time taken to finish modelling:",
              "{:.3f}".format(t2-t1), "seconds.")
        print("Saved in:", outdir_ised)
