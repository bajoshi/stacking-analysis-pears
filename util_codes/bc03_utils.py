...
def call_cspgalaxev(isedfile, tau, output, dust='N', z='0', sfh='1', recycle='N', tcut='20.0'):
    """
    ...
    start = subprocess.Popen([home + '/Documents/GALAXEV2016/bc03/src/csp_galaxev'], stdin=subprocess.PIPE, encoding='ascii')
...
def get_bc03_spectrum(age, tau, metals, outdir_ised, save2ascii=True):
    ...
    # The weird construction of the tau string here is because 
    # csp_galaxev does not like to be given output files with
    # periods in them. This contruction ensures that we get rid 
    # of the period while also not losing information, i.e., based
    # on the filename we can later infer what the tau value was.
    ...
    if not os.path.isfile(output + '.ised'):
        ...
        print("\nOutput ised file exists. Moving on to age extraction.")
    ...
    ised2fits(output + '.ised', del_modelfile=True)
